...
def train_one_epoch_for_enc1(model,pbar,train_iterator,train_loader,os,step,recent_losses,device):
...
def eval_one_epoch_for_enc1(model,valid_loader,device):
    ...
    with torch.no_grad():
        ...
        for batch in valid_loader:
            ...
            seq = batch["seq"].to(device)
            mask = batch["mask"].to(device)
            # seq3 = batch3["seq"].to(device)
            # mask3 = batch3["mask"].to(device)
...
def load_parameters(msd,sd1,sd3):

    def load(msd,sd1,sd3,prefix,suffix,num):
        if num ==1:
            
            msd[prefix+"1."+suffix].copy_(sd1[prefix+suffix])
            msd[prefix+"2."+suffix].copy_(sd3[prefix+suffix])
        else:
            for i in range(num):    
                i=str(i)
                
                msd[prefix+"1."+i+suffix].copy_(sd1[prefix+i+suffix])
                msd[prefix+"2."+i+suffix].copy_(sd3[prefix+i+suffix])
    
    load(msd,sd1,sd3,"decoder.net.token_emb.",".emb.weight",5)
    
    load(msd,sd1,sd3,"decoder.net.pos_emb.","emb.weight",1)
    
    pre="decoder.net.attn_layers."
    for key in msd.keys():
        if pre in key:
            load(msd,sd1,sd3,pre,key[len(pre)+2:],1)
...
def main():
    ...
    if args.dataset is not None:
        ...
        if args.out_dir is None:
            args.out_dir = pathlib.Path(f"exp/test_enc123_{args.dataset}")
    ...
    f.close()
    '''
    path1="/work100/weixp/mtmt3-paper-2/mtmt3-final-local_attetion/exp/lmd/ape/checkpoints_enc3/best_model_enc3.pt"
    path2="/work100/weixp/mtmt3-paper-2/mtmt3-final-local_attetion/exp/lmd-track/ape/checkpoints_enc3/best_model_enc3.pt"
    load_parameters(model.state_dict(),torch.load(path1),torch.load(path2))
    '''
    f=open("msd.txt","w")
    ...
    while step < args.steps:
        ...
        step,seq,mask,train_loss = train_one_epoch_for_enc1(model,pbar,train_iterator,train_loader,os,step,recent_losses,device)
        ...
        model.eval()
        total_loss,total_losses,count,seq,mask = eval_one_epoch_for_enc1(model,valid_loader,device)
