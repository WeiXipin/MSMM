...
def train_one_epoch_for_enc3(model,pbar,train_iterator,train_loader,os,step,recent_losses):
...
def eval_one_epoch_for_enc3(model,valid_loader):
    device = torch.device(
        f"cuda:3"
    )
    with torch.no_grad():
...
def main():
    ...
    if args.dataset is not None:
        ...
        if args.out_dir is None:
            args.out_dir = pathlib.Path(f"exp/test_enc3_{args.dataset}")

    # Make sure the output directory exists
    args.out_dir.mkdir(exist_ok=True)
    (args.out_dir / "checkpoints_enc3").mkdir(exist_ok=True)
    ...
    model = music_x_transformers3.MusicXTransformer3(
        dim=args.dim,
        encoding=encoding_list[2],
        depth=args.layers,
        heads=args.heads,
        max_seq_len=args.max_seq_len,
        max_beat=args.max_beat,
    ...
    train_iterator = iter(train_loader_list[2])
    while step < args.steps:
        ...
        step,seq,mask,train_loss = train_one_epoch_for_enc3(model,pbar,train_iterator,train_loader_list[2],os,step,recent_losses)
        ...
        model.eval()
        total_loss,total_losses,count,seq,mask = eval_one_epoch_for_enc3(model,valid_loader_list[2])
        ...
        # Save the model
        checkpoint_filename = args.out_dir / "checkpoints_enc3" / f"modele_nc3_{step}.pt"
        ...
        if val_loss < min_val_loss:
            ...
            shutil.copyfile(
                checkpoint_filename,
                args.out_dir / "checkpoints_enc3" / "best_model_enc3.pt",
            )
    ...
    optimizer_filename = args.out_dir / "checkpoints_enc3" / f"optimizer_enc3_{step}.pt"
    ...
    scheduler_filename = args.out_dir / "checkpoints_enc3" / f"scheduler_enc3_{step}.pt"
