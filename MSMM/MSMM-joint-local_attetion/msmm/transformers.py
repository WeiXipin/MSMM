...
class MusicTransformerWrapper(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        _,out2=process(x1,1,mask,mems)
        _,out3=process(x2,2,mask,mems)
...
class MusicAutoregressiveWrapper(nn.Module):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if num_dims == 2:
            ...
            start_tokens1 = start_tokens1[None, :, :]
            start_tokens2 = start_tokens2[None, :, :]
        self.net.eval()
        out = start_tokens
        out1 = start_tokens1
        out2 = start_tokens2
        ...
        for _ in range(seq_len):
            ...
            x1 = out1[:, -self.max_seq_len :]
            x2 = out2[:, -self.max_seq_len :]
            mask = mask[:, -self.max_seq_len :]

            if return_attn:
                logits, attn = self.net(
                    x,x1,x2, mask=mask, return_attn=True, **kwargs
                )
                logits = [l[:, -1, :] for l in logits]
            else:
                logits = [
                    l[:, -1, :] for l in self.net(x,x1,x2, mask=mask, **kwargs)
                ]
            ...
            out1 = torch.cat((out1, stacked), dim=1)
            out2 = torch.cat((out2, stacked), dim=1)
            ...
            if exists(eos_token):
                ...
                is_eos_tokens1 = out1[..., 0] == eos_token
                # Mask out everything after the eos tokens
                if is_eos_tokens1.any(dim=1).all():
                    for i, is_eos_token in enumerate(is_eos_tokens1):
                        idx = torch.argmax(is_eos_token.byte())
                        out1[i, idx + 1 :] = self.pad_value
                    break
                
                is_eos_tokens2 = out2[..., 0] == eos_token
                # Mask out everything after the eos tokens
                if is_eos_tokens2.any(dim=1).all():
                    for i, is_eos_token in enumerate(is_eos_tokens2):
                        idx = torch.argmax(is_eos_token.byte())
                        out2[i, idx + 1 :] = self.pad_value
                    break
    ...
    def forward(self, x,x1,x2, return_list=False, **kwargs):
        xi = x[:, :-1]
        xi1 = x1[:, :-1]
        xi2 = x2[:, :-1]
        xo = x[:, 1:]
        ...
        out = self.net(xi,xi1,xi2,**kwargs)
        losses = [
            F.cross_entropy(
                out[i].transpose(1, 2),
                xo[..., i],
...
class MusicXTransformer(nn.Module):
    ...
    @torch.no_grad()
    def generate(self, seq_in,seq_in1,seq_in2, seq_len, **kwargs):
        return self.decoder.generate(seq_in,seq_in1,seq_in2, seq_len, **kwargs)

    def forward(self, seq,seq1,seq2, mask=None, **kwargs):
        return self.decoder(seq,seq1,seq2, mask=mask, **kwargs)
